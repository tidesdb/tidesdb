...
- Two-tier caching system for optimal performance:
  ...
  - **Block Cache** · Lock-free clock cache for deserialized entries shared across all column families and SSTables. Uses atomic operations with CLOCK eviction algorithm (second-chance page replacement) for true lock-free concurrent access without read-write locks. Configurable memory budget (default 64MB) with dynamic hash table sizing (power-of-2 buckets for fast lookups). Cache keys use format `cf_name:sstable_id:key` for precise entry identification. Lock-free memory pool eliminates malloc overhead in hot paths (~5ns vs ~50ns). Decompressed entries are cached on first read for instant subsequent access with automatic eviction under memory pressure. Reference bit tracking gives recently accessed entries a "second chance" before eviction, significantly reducing disk I/O and decompression CPU overhead for hot data while maintaining thread-safety across iterators, GET operations, and compaction.
