...
## Features
- Data-structures are lock-free with atomic CAS operations. Reads are completely lock-free and scale linearly with CPU cores. Writes use atomic CAS to prepend new versions without locks. Immutable memtable queue ensures data remains searchable during background flush operations, guaranteeing consistent reads without blocking.
...
- Efficient seek operations using O(log n) skip list positioning and optional block indexes for prefix boundary to block mapping in SSTables. Block indexes sample every 16th block min-max prefix boundary during SSTable creation to build a sparse index that maps sampled ranges to block numbers to optimize reads. Configurable prefix length (default 16 bytes) balances index size with seek precision.
...
- Block manager with overflow chaining for large values. Max inline blocks (default 64KB) prevent fragmentation while overflow pointers chain multiple blocks for values exceeding block size.
- Two-tier caching system for optimal performance:
  - **SSTable Cache** · Hybrid LRU/LFU cache (70% LRU, 30% LFU) manages file descriptor limits by caching open block managers (klog/vlog file handles) with configurable capacity (default 512 SSTables). Blocks opening more than max_open_sstables with exponential backoff retry (up to 100 attempts) when cache is full, enforcing strict file descriptor limits. Blocks promoted to LFU tier after 3 accesses with 5-minute TTL for frequently accessed SSTables. Automatic eviction closes file handles via callback when SSTables are removed from cache. Level-by-level search with early termination optimizes reads by checking bloom filters and range bounds before taking SSTable references, minimizing file handle contention. Position cache built on SSTable load enables O(1) cursor navigation (goto_last, cursor_prev) instead of O(n) scans, preventing hangs during metadata/bloom/index loading.
  - **Block Cache** · Lock-free CLOCK cache for hot SSTable data with automatic configuration based on CPU count and memory budget. Uses atomic operations with CLOCK eviction algorithm (second-chance page replacement) for true lock-free concurrent reads and writes without mutexes. Partitioned design (1 partition per CPU core, up to 128 partitions) minimizes contention with thread-local clock hands. Configurable memory budget (default 64MB) with power-of-2 slot sizing for fast modulo operations. Cache keys use format `cf_name:key` (column family scoped, SSTable ID irrelevant) ensuring cache survives compaction without invalidation. Read-optimized design: cache is checked after memtables but before SSTables, ensuring latest data is always returned while avoiding disk I/O for hot keys. Cache is populated only on SSTable reads (not writes) to prevent pollution from write-once keys. Automatic cache warming during WAL recovery pre-populates cache with recently written keys for instant performance on restart. Reference bit tracking gives recently accessed entries a "second chance" before eviction. Hybrid hash table + circular array design enables O(1) lookups with linear probing (max 64 probes) and efficient CLOCK scanning. Significantly reduces disk I/O and decompression CPU overhead for random read workloads while maintaining thread-safety across concurrent transactions and compaction.
