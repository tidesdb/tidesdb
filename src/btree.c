...
/**
 * btree_node_deserialize
 * deserializes a node from optimized format:
 * -- varint encoded sizes and metadata
 * -- prefix-compressed keys (leaf nodes)
 * -- delta-encoded child offsets (internal nodes)
 ...
 * @return 0 on success, -1 on failure
 */
static int btree_node_deserialize(const uint8_t *data, const size_t data_size, btree_node_t **node)
{
    if (!data || data_size < 2 || !node) return -1;

    btree_node_t *n = calloc(1, sizeof(btree_node_t));
    if (!n) return -1;
    ...
    if (n->type == BTREE_NODE_LEAF)
    {
        ...
        if (n->num_entries > 0)
        {
            n->entries = calloc(n->num_entries, sizeof(btree_entry_t));
            n->keys = calloc(n->num_entries, sizeof(uint8_t *));
            n->key_sizes = calloc(n->num_entries, sizeof(size_t));
            n->values = calloc(n->num_entries, sizeof(uint8_t *));

            if (!n->entries || !n->keys || !n->key_sizes || !n->values)
            {
                btree_node_free(n);
                return -1;
            }

            uint16_t *key_offsets = malloc(n->num_entries * sizeof(uint16_t));
            if (!key_offsets)
            {
                btree_node_free(n);
                return -1;
            ...
            size_t *prefix_lens = malloc(n->num_entries * sizeof(size_t));
            size_t *suffix_lens = malloc(n->num_entries * sizeof(size_t));
            if (!prefix_lens || !suffix_lens)
            {
                free(key_offsets);
                free(prefix_lens);
            ...
            for (uint32_t i = 0; i < n->num_entries; i++)
            {
                n->keys[i] = malloc(n->entries[i].key_size);
                if (!n->keys[i])
                {
                    free(key_offsets);
                    free(prefix_lens);
            ...
            for (uint32_t i = 0; i < n->num_entries; i++)
            {
                ...
                {
                    n->values[i] = malloc(n->entries[i].value_size);
                    if (!n->values[i])
                    {
                        btree_node_free(n);
                        return -1;
    ...
    else if (n->type == BTREE_NODE_INTERNAL)
    {
        ...
        n->child_offsets = calloc(num_children, sizeof(int64_t));
        n->keys = calloc(num_keys, sizeof(uint8_t *));
        n->key_sizes = calloc(num_keys, sizeof(size_t));

        if (!n->child_offsets || (num_keys > 0 && (!n->keys || !n->key_sizes)))
        {
            btree_node_free(n);
            return -1;
        }
