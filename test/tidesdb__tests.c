...
static void test_multiple_column_families(void)
{
    ...
    /* we verify isolation between CFs */
    tidesdb_txn_t *txn3 = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn3), 0);
    uint8_t *retrieved_value = NULL;
    size_t retrieved_size = 0;
...
static void test_memtable_flush(void)
{
    ...
    /* we verify all data is still accessible */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_persistence_and_recovery(void)
{
    ...
    /* we create database, write data, flush, close */
    {
        tidesdb_t *db = create_test_db();
        tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
        cf_config.compression_algorithm = TDB_COMPRESS_NONE;
    ...
    /* we reopen database and verify all data persisted */
    {
        tidesdb_config_t config = tidesdb_default_config();
        config.db_path = TEST_DB_PATH;
...
static void test_checkpoint_basic(void)
{
    ...
    /* we write more data (will be in memtable, checkpoint should flush it) */
    for (int i = 50; i < 80; i++)
    {
        tidesdb_txn_t *txn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    ...
    /* we open the checkpoint as a database and verify all data */
    tidesdb_config_t config = tidesdb_default_config();
...
static void test_checkpoint_data_integrity_after_compaction(void)
{
    ...
    /* we write enough data across multiple flushes to create multiple sstables */
    ...
    /* we create checkpoint after compaction */
    ASSERT_EQ(tidesdb_checkpoint(db, TEST_DB_CHECKPOINT_PATH), 0);
    ASSERT_EQ(tidesdb_close(db), 0);

    /* we open checkpoint and verify all 120 keys */
    tidesdb_config_t config = tidesdb_default_config();
    config.db_path = TEST_DB_CHECKPOINT_PATH;
...
static void test_checkpoint_concurrent_writes(void)
{
    ...
    /* we pre-populate some data and flush */
    for (int i = 0; i < 50; i++)
    {
        tidesdb_txn_t *txn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    ...
    /* we start a writer thread that writes concurrently */
    ...
    /* we open checkpoint and verify pre-populated data is accessible */
    tidesdb_config_t config = tidesdb_default_config();
    config.db_path = TEST_DB_CHECKPOINT_PATH;
...
static void test_checkpoint_multi_cf(void)
{
    ...
    /* we open checkpoint and verify both CFs */
    tidesdb_config_t config = tidesdb_default_config();
    config.db_path = TEST_DB_CHECKPOINT_PATH;
...
static void test_stats_comprehensive(void)
{
    ...
    /* we verify level arrays are properly allocated */
    ...
    /* we verify we have data in ssts now */
    ASSERT_TRUE(stats->total_keys > 0);
    ASSERT_TRUE(stats->total_data_size > 0);

    /* we verify read amplification calculation */
...
static void test_compaction_with_deletes(void)
{
    ...
    /* we write enough data to exceed level 0 capacity and trigger level addition
    ...
    /* we verify deleted keys are gone and remaining keys exist */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_ttl_expiration(void)
{
    ...
    /* we verify key exists immediately */
    txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_isolation_serializable(void)
{
    ...
    /* we create dangerous structure with SERIALIZABLE transactions:
     * T1 reads X, writes Y
     * T2 reads Y, writes X
    ...
    /* we try to commit both -- at least one must fail to prevent serialization anomaly */
...
static void test_clone_column_family(void)
{
    ...
    /* we verify new key does not exist in source */
    retrieved_value = NULL;
    retrieved_size = 0;
    ...
    /* we verify clone-only key still exists after reopen */
    retrieved_value = NULL;
    retrieved_size = 0;
    ASSERT_EQ(tidesdb_txn_get(read_txn, clone_cf, new_key, sizeof(new_key), &retrieved_value,
                              &retrieved_size),
...
static void *rename_concurrent_writer(void *arg)
{
    ...
    for (int i = 0; i < ctx->num_ops; i++)
    {
        /* we check if rename is done, if so get new CF handle */
...
static void test_empty_iterator(void)
{
    ...
    /* we create iterator on empty column family */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_block_indexes(void)
{
    ...
    /* we use iterator seek (should use block index) */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_block_indexes_sparse_sample_ratio(void)
{
    ...
    /* we flush to create sstable with sparse block index */
    tidesdb_flush_memtable(cf);
    usleep(200000);
...
static void test_sync_interval_mode(void)
{
    ...
    /* test TDB_SYNC_INTERVAL with periodic syncing */
    {
        ...
        /* we verify data is readable immediately (from memtable) */
        tidesdb_txn_t *txn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
        uint8_t *retrieved_value = NULL;
        size_t retrieved_size = 0;
        ...
        /* we wait for at least one sync interval to ensure WAL is synced */
        usleep(600000);
    ...
    /* test multiple CFs with different intervals */
    {
        ...
        /* we verify all data is readable */
        tidesdb_txn_t *txn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
        uint8_t *retrieved_value = NULL;
        size_t retrieved_size = 0;
...
static void test_empty_value(void)
{
    ...
    /* we verify empty value can be retrieved */
    tidesdb_txn_t *txn2 = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn2), 0);
    uint8_t *retrieved_value = NULL;
    size_t retrieved_size = 0;
...
static void test_delete_nonexistent_key(void)
{
    ...
    /* we delete a key that doesn't exist -- should succeed (idempotent) */
    uint8_t key[] = "nonexistent_key";
    ASSERT_EQ(tidesdb_txn_delete(txn, cf, key, sizeof(key)), 0);
    ASSERT_EQ(tidesdb_txn_commit(txn), 0);

    /* we try to get the deleted key -- should not exist */
    tidesdb_txn_t *txn2 = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn2), 0);
    uint8_t *value = NULL;
    size_t value_size = 0;
...
static void test_overwrite_same_key_multiple_times(void)
{
    ...
    /* we overwrite the same key 100 times */
    for (int i = 0; i < 100; i++)
    {
        tidesdb_txn_t *txn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    ...
    /* we verify we get the last value */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    uint8_t *retrieved_value = NULL;
    size_t retrieved_size = 0;
...
static void test_put_delete_put_same_key(void)
{
    ...
    /* we verify we get the second value */
    tidesdb_txn_t *txn4 = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn4), 0);
    uint8_t *retrieved_value = NULL;
    size_t retrieved_size = 0;
...
static void test_iterator_single_key(void)
{
    ...
    /* we try to go next -- should return NOT_FOUND and become invalid */
...
static void test_read_across_multiple_sstables(void)
{
    ...
    /* we wait for flushes to complete */
    usleep(100000);

    /* we verify all keys can be read correctly from different ssts */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_read_with_bloom_filter_disabled(void)
{
    ...
    /* we verify reads work without bloom filter */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_read_with_block_indexes_disabled(void)
{
    ...
    /* we verify reads work without block indexes */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_read_with_all_optimizations_disabled(void)
{
    ...
    /* we verify reads work with all optimizations disabled */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_iterator_no_bloom_no_indexes(void)
{
    ...
    /* we test iterator on flushed data */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
...
static void test_multi_cf_transaction_recovery_comprehensive(void)
{
    ...
    /* now reopen database and verify recovery */
    {
        ...
        /* we verify rolled back transactions in cf_mixed are not present */
        for (int i = 0; i < NUM_ROLLBACK_TXNS; i++)
        {
            char key[32];
            snprintf(key, sizeof(key), "mixed_rollback_%03d", i);
...
static void test_commit_hook_error_does_not_rollback(void)
{
    ...
    /* we verify data is still accessible */
    tidesdb_txn_t *txn2 = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn2), 0);
    uint8_t *retrieved_value = NULL;
    size_t retrieved_size = 0;
...
static void test_memory_pressure_level_computation(void)
{
     ...
     * must be done after CF creation because CF creation validates write_buffer_size
     * against resolved_memory_limit */
    ...
    for (int i = 0; i < 800; i++)
    {
        /* we keep writing to maintain memtable pressure across reaper cycles */
        for (int w = 0; w < 5; w++)
        {
            tidesdb_txn_t *txn = NULL;
...
static void test_memory_pressure_force_flush_and_compaction(void)
{
    ...
    /* we write data in rounds, flushing to create multiple sstables
...
static void test_memory_pressure_auto_limit(void)
{
    ...
    /* we verify absurdly low max_memory_usage gets clamped to 5% floor */
    cleanup_test_dir();
    config = tidesdb_default_config();
    config.db_path = TEST_DB_PATH;
    ...
    size_t floor = (size_t)((double)total_mem * 0.05);
...
typedef struct
{
    ...
    _Atomic(int) *go;
    _Atomic(int) *stop;
} iter_direction_thread_data_t;

static void *iter_seek_last_thread(void *arg)
{
    iter_direction_thread_data_t *d = (iter_direction_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    }
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        tidesdb_iter_t *iter = NULL;
        if (tidesdb_iter_new(txn, d->cf, &iter) != 0)
        {
            tidesdb_txn_free(txn);
            continue;
        }
        ...
        int walked = 0;
        while (tidesdb_iter_valid(iter) && walked < 5)
        {
            uint8_t *key = NULL;
            size_t ks = 0;
            if (tidesdb_iter_key(iter, &key, &ks) != 0) break;
            tidesdb_iter_prev(iter);
            walked++;
        }

        char seek_key[64];
        snprintf(seek_key, sizeof(seek_key), "mdir_key_%06d", (i * 7) % 200);
...
static void *iter_seek_forward_thread(void *arg)
{
    iter_direction_thread_data_t *d = (iter_direction_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        char seek_key[64];
        snprintf(seek_key, sizeof(seek_key), "mdir_key_%06d", (i * 13) % 200);
        tidesdb_iter_seek(iter, (uint8_t *)seek_key, strlen(seek_key) + 1);
        int walked = 0;
        while (tidesdb_iter_valid(iter) && walked < 10)
        {
            uint8_t *key = NULL;
            size_t ks = 0;
            if (tidesdb_iter_key(iter, &key, &ks) != 0) break;
        ...
        snprintf(seek_key, sizeof(seek_key), "mdir_key_%06d", (i * 17) % 200);
        tidesdb_iter_seek_for_prev(iter, (uint8_t *)seek_key, strlen(seek_key) + 1);
        walked = 0;
        while (tidesdb_iter_valid(iter) && walked < 5)
        {
...
static void *iter_direction_writer_thread(void *arg)
{
    iter_direction_thread_data_t *d = (iter_direction_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    {
        tidesdb_txn_t *txn = NULL;
        if (tidesdb_txn_begin(d->db, &txn) != 0) continue;

        char key[64], value[256];
        snprintf(key, sizeof(key), "mdir_key_%06d", i % 200);
        memset(value, 'W' + (d->thread_id % 10), sizeof(value));
        snprintf(value, sizeof(value), "t%d_i%d", d->thread_id, i);

        tidesdb_txn_put(txn, d->cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                        strlen(value) + 1, 0);

        if (i % 7 == 0)
        {
            char del_key[64];
            snprintf(del_key, sizeof(del_key), "mdir_key_%06d", (i + 100) % 200);
            tidesdb_txn_delete(txn, d->cf, (uint8_t *)del_key, strlen(del_key) + 1);
        }
...
static void test_concurrent_iter_seek_directions(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ...
    ASSERT_EQ(tidesdb_create_column_family(db, "mdir_cf", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "mdir_cf");
    ASSERT_TRUE(cf != NULL);
    ...
    for (int i = 0; i < 200; i++)
    {
        ...
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
        char key[64], value[128];
        snprintf(key, sizeof(key), "mdir_key_%06d", i);
        snprintf(value, sizeof(value), "mdir_value_%06d", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                                  strlen(value) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    ...
    const int NUM_SEEK_LAST = 4;
    const int NUM_SEEK_FWD = 4;
    const int NUM_WRITERS = 2;
    const int OPS = 300;

    _Atomic(int) errors = 0, go = 0, stop = 0;

    pthread_t seek_last_threads[4], seek_fwd_threads[4], writer_threads[2];
    iter_direction_thread_data_t sl_data[4], sf_data[4], wr_data[2];

    for (int i = 0; i < NUM_SEEK_LAST; i++)
    {
        sl_data[i] = (iter_direction_thread_data_t){db, cf, i, OPS, &errors, &go, &stop};
        pthread_create(&seek_last_threads[i], NULL, iter_seek_last_thread, &sl_data[i]);
    }
    for (int i = 0; i < NUM_SEEK_FWD; i++)
    {
        sf_data[i] = (iter_direction_thread_data_t){db, cf, 10 + i, OPS, &errors, &go, &stop};
        pthread_create(&seek_fwd_threads[i], NULL, iter_seek_forward_thread, &sf_data[i]);
    }
    for (int i = 0; i < NUM_WRITERS; i++)
    {
        wr_data[i] = (iter_direction_thread_data_t){db, cf, 20 + i, OPS * 2, &errors, &go, &stop};
        pthread_create(&writer_threads[i], NULL, iter_direction_writer_thread, &wr_data[i]);
    }

    atomic_store(&go, 1);

    for (int i = 0; i < NUM_WRITERS; i++) pthread_join(writer_threads[i], NULL);
    atomic_store(&stop, 1);
    for (int i = 0; i < NUM_SEEK_LAST; i++) pthread_join(seek_last_threads[i], NULL);
    for (int i = 0; i < NUM_SEEK_FWD; i++) pthread_join(seek_fwd_threads[i], NULL);

    int final_errors = atomic_load(&errors);
    printf("  concurrent iter seek directions: errors=%d\n", final_errors);
    ASSERT_EQ(final_errors, 0);
...
typedef struct
{
    ...
    tidesdb_column_family_t *idx_cf;
    int thread_id;
    int num_ops;
    _Atomic(int) *errors;
    _Atomic(int) *go;
    _Atomic(int) *stop;
} multi_cf_thread_data_t;

static void *multi_cf_writer_thread(void *arg)
{
    multi_cf_thread_data_t *d = (multi_cf_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    }
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        char pk[64], row_val[128], idx_key[128];
        snprintf(pk, sizeof(pk), "\x01pk_%04d_%04d", d->thread_id, i % 100);
        snprintf(row_val, sizeof(row_val), "row_t%d_i%d", d->thread_id, i);
        snprintf(idx_key, sizeof(idx_key), "\x01idx_v%04d_pk_%04d_%04d", i % 50, d->thread_id,
                 i % 100);

        /* we write data CF + index CF atomically in same txn */
        tidesdb_txn_put(txn, d->data_cf, (uint8_t *)pk, strlen(pk) + 1, (uint8_t *)row_val,
                        strlen(row_val) + 1, 0);
        tidesdb_txn_put(txn, d->idx_cf, (uint8_t *)idx_key, strlen(idx_key) + 1, (uint8_t *)"\x00",
                        1, 0);

        /* interleave deletes */
        if (i % 11 == 0 && i > 0)
        {
            char old_pk[64], old_idx[128];
            snprintf(old_pk, sizeof(old_pk), "\x01pk_%04d_%04d", d->thread_id, (i - 5) % 100);
            snprintf(old_idx, sizeof(old_idx), "\x01idx_v%04d_pk_%04d_%04d", (i - 5) % 50,
                     d->thread_id, (i - 5) % 100);
            tidesdb_txn_delete(txn, d->data_cf, (uint8_t *)old_pk, strlen(old_pk) + 1);
...
static void *multi_cf_reader_thread(void *arg)
{
    multi_cf_thread_data_t *d = (multi_cf_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        /* we alternate between reading data CF and index CF */
        tidesdb_column_family_t *cf = (i % 2 == 0) ? d->data_cf : d->idx_cf;
        ...
        {
            char seek[64];
            snprintf(seek, sizeof(seek), "\x01pk_%04d", (i * 7) % 100);
            tidesdb_iter_seek(iter, (uint8_t *)seek, strlen(seek) + 1);
        }

        int walked = 0;
        while (tidesdb_iter_valid(iter) && walked < 20)
        {
            uint8_t *key = NULL;
            size_t ks = 0;
...
static void test_concurrent_multi_cf_put_delete_iterate(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ...
    ASSERT_EQ(tidesdb_create_column_family(db, "mcf_data", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "mcf_idx", &cf_config), 0);
    tidesdb_column_family_t *data_cf = tidesdb_get_column_family(db, "mcf_data");
    tidesdb_column_family_t *idx_cf = tidesdb_get_column_family(db, "mcf_idx");
    ASSERT_TRUE(data_cf != NULL);
    ASSERT_TRUE(idx_cf != NULL);
    ...
    multi_cf_thread_data_t w_data[4], r_data[4];

    for (int i = 0; i < NUM_WRITERS; i++)
    {
        w_data[i] = (multi_cf_thread_data_t){db, data_cf, idx_cf, i, OPS, &errors, &go, &stop};
        pthread_create(&writers[i], NULL, multi_cf_writer_thread, &w_data[i]);
    }
    for (int i = 0; i < NUM_READERS; i++)
    {
        r_data[i] =
            (multi_cf_thread_data_t){db, data_cf, idx_cf, 10 + i, OPS * 2, &errors, &go, &stop};
        pthread_create(&readers[i], NULL, multi_cf_reader_thread, &r_data[i]);
    }
    ...
    int final_errors = atomic_load(&errors);
    printf("  concurrent multi-CF put/delete/iterate: errors=%d\n", final_errors);
    ASSERT_EQ(final_errors, 0);
...
static void test_iterator_reuse_seek_patterns(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ...
    ASSERT_EQ(tidesdb_create_column_family(db, "reuse_cf", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "reuse_cf");
    ASSERT_TRUE(cf != NULL);
    ...
    for (int i = 0; i < 150; i++)
    {
        ...
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
        char key[64], value[128];
        snprintf(key, sizeof(key), "reuse_key_%06d", i);
        snprintf(value, sizeof(value), "reuse_value_%06d", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                                  strlen(value) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    ...
    /* we drain flushes to ensure data is in sstables */
    for (int i = 0; i < 100; i++)
    {
        if (!atomic_load(&cf->is_flushing) && queue_size(db->flush_queue) == 0) break;
        usleep(20000);
    ...
    /* we create one iterator and reuse it across many seek patterns */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    ...
    for (int round = 0; round < 50; round++)
    {
        /* pat 1 -- seek forward */
        char seek_key[64];
        snprintf(seek_key, sizeof(seek_key), "reuse_key_%06d", (round * 3) % 150);
        tidesdb_iter_seek(iter, (uint8_t *)seek_key, strlen(seek_key) + 1);
        if (tidesdb_iter_valid(iter))
        {
            uint8_t *key = NULL;
        ...
        /* pat 3 -- seek_for_prev */
        snprintf(seek_key, sizeof(seek_key), "reuse_key_%06d", (round * 7 + 50) % 150);
        tidesdb_iter_seek_for_prev(iter, (uint8_t *)seek_key, strlen(seek_key) + 1);
        if (tidesdb_iter_valid(iter))
        {
            uint8_t *key = NULL;
        ...
        /* pat 4 -- we immediately seek to first (rnd_init full table scan) */
        tidesdb_iter_seek_to_first(iter);
        int walked = 0;
        while (tidesdb_iter_valid(iter) && walked < 3)
        {
            tidesdb_iter_next(iter);
            walked++;
        }

        /* pat 5 -- we immediately seek forward again (next statement reuse) */
        snprintf(seek_key, sizeof(seek_key), "reuse_key_%06d", (round * 11) % 150);
        tidesdb_iter_seek(iter, (uint8_t *)seek_key, strlen(seek_key) + 1);
        if (tidesdb_iter_valid(iter))
        {
            uint8_t *key = NULL;
    ...
    printf("  iterator reuse: 50 rounds of 5 seek patterns completed\n");
...
static void test_concurrent_savepoint_rollback_under_load(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ...
    for (int i = 0; i < 50; i++)
    {
        ...
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
        char key[64], value[64];
        snprintf(key, sizeof(key), "sp_key_%04d", i);
        snprintf(value, sizeof(value), "sp_val_%04d", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                                  strlen(value) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    ...
    int savepoint_ok = 0, rollback_ok = 0, commit_ok = 0;

    /* we run many txns with savepoint/rollback patterns */
    for (int round = 0; round < 200; round++)
    {
        tidesdb_txn_t *txn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);

        /* stmt 1 -- we successful write with savepoint */
        int sp_rc = tidesdb_txn_savepoint(txn, "stmt");
        if (sp_rc == 0) savepoint_ok++;

        char key[64], value[64];
        snprintf(key, sizeof(key), "sp_round_%04d_s1", round);
        snprintf(value, sizeof(value), "val_%04d_s1", round);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                                  strlen(value) + 1, 0),
                  0);

        tidesdb_txn_release_savepoint(txn, "stmt");

        /* stmt 2 -- we write then rollback */
        sp_rc = tidesdb_txn_savepoint(txn, "stmt");
        if (sp_rc == 0) savepoint_ok++;

        snprintf(key, sizeof(key), "sp_round_%04d_SHOULD_NOT_EXIST", round);
        snprintf(value, sizeof(value), "rollback_me");
        tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                        strlen(value) + 1, 0);

        /* simulate statement failure -- rollback to savepoint */
        int rb_rc = tidesdb_txn_rollback_to_savepoint(txn, "stmt");
        if (rb_rc == 0) rollback_ok++;

        /* stmt 3 -- another successful write after rollback */
        sp_rc = tidesdb_txn_savepoint(txn, "stmt");

        snprintf(key, sizeof(key), "sp_round_%04d_s3", round);
        snprintf(value, sizeof(value), "val_%04d_s3", round);
        tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                        strlen(value) + 1, 0);

        tidesdb_txn_release_savepoint(txn, "stmt");

        /* we commit the whole txn */
        if (tidesdb_txn_commit(txn) == 0) commit_ok++;
        tidesdb_txn_free(txn);
    }

    printf("  savepoint/rollback: %d savepoints, %d rollbacks, %d commits\n", savepoint_ok,
           rollback_ok, commit_ok);
    ASSERT_EQ(commit_ok, 200);

    /* s1 and s3 keys should exist, SHOULD_NOT_EXIST keys should not */
    tidesdb_txn_t *vtxn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &vtxn), 0);

    int found_s1 = 0, found_s3 = 0, found_bad = 0;
    for (int round = 0; round < 200; round++)
    {
        char key[64];
        uint8_t *val = NULL;
    ...
    printf("  verification: s1=%d/200, s3=%d/200, bad=%d (should be 0)\n", found_s1, found_s3,
           found_bad);
    ASSERT_EQ(found_s1, 200);
    ASSERT_EQ(found_s3, 200);
    ASSERT_EQ(found_bad, 0);

    tidesdb_close(db);
    cleanup_test_dir();
}

static void test_concurrent_iter_seek_directions_deep_sst(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ...
    cf_config.enable_bloom_filter = 1;
    cf_config.enable_block_indexes = 1;
    cf_config.klog_value_threshold = 256;

    ASSERT_EQ(tidesdb_create_column_family(db, "deep_dir_cf", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "deep_dir_cf");
    ASSERT_TRUE(cf != NULL);

    /* seed data in batches with flush+compact between to push ssts deep */
    for (int batch = 0; batch < 5; batch++)
    {
        for (int i = 0; i < 200; i++)
        {
            int key_idx = batch * 200 + i;
            tidesdb_txn_t *txn = NULL;
            ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
            char key[64], value[256];
            snprintf(key, sizeof(key), "ddir_key_%06d", key_idx);
            /* we use values > klog_value_threshold to exercise vlog path too */
            memset(value, 'A' + (batch % 26), sizeof(value));
            snprintf(value, sizeof(value), "ddir_batch%d_idx%d_padding_to_fill_block_", batch, i);
    ...
    /* we verify we have ssts across multiple levels */
    int num_levels = atomic_load(&cf->num_active_levels);
    int total_ssts = 0;
    int levels_with_ssts = 0;
    for (int l = 0; l < num_levels; l++)
    {
        if (cf->levels[l])
        {
            int n = atomic_load(&cf->levels[l]->num_sstables);
            if (n > 0) levels_with_ssts++;
            total_ssts += n;
        }
    }
    printf("  deep SST seed: %d levels, %d total SSTables, %d levels with data\n", num_levels,
           total_ssts, levels_with_ssts);
    ASSERT_TRUE(total_ssts >= 1);

    /* we run a concurrent seek stress with data on disk across levels */
    const int NUM_SEEK_LAST = 4;
    const int NUM_SEEK_FWD = 4;
    const int NUM_WRITERS = 2;
    const int OPS = 200;
    ...
    pthread_t sl_threads[4], sf_threads[4], wr_threads[2];
    iter_direction_thread_data_t sl_data[4], sf_data[4], wr_data[2];

    for (int i = 0; i < NUM_SEEK_LAST; i++)
    {
        sl_data[i] = (iter_direction_thread_data_t){db, cf, i, OPS, &errors, &go, &stop};
        pthread_create(&sl_threads[i], NULL, iter_seek_last_thread, &sl_data[i]);
    }
    for (int i = 0; i < NUM_SEEK_FWD; i++)
    {
        sf_data[i] = (iter_direction_thread_data_t){db, cf, 10 + i, OPS, &errors, &go, &stop};
        pthread_create(&sf_threads[i], NULL, iter_seek_forward_thread, &sf_data[i]);
    }
    for (int i = 0; i < NUM_WRITERS; i++)
    {
        wr_data[i] = (iter_direction_thread_data_t){db, cf, 20 + i, OPS * 3, &errors, &go, &stop};
        pthread_create(&wr_threads[i], NULL, iter_direction_writer_thread, &wr_data[i]);
    }

    atomic_store(&go, 1);

    for (int i = 0; i < NUM_WRITERS; i++) pthread_join(wr_threads[i], NULL);
    atomic_store(&stop, 1);
    for (int i = 0; i < NUM_SEEK_LAST; i++) pthread_join(sl_threads[i], NULL);
    for (int i = 0; i < NUM_SEEK_FWD; i++) pthread_join(sf_threads[i], NULL);

    int final_errors = atomic_load(&errors);
    printf("  concurrent iter seek directions (deep SST): errors=%d\n", final_errors);
    ASSERT_EQ(final_errors, 0);
    ...
    tidesdb_txn_free(vtxn);
    printf("  post-stress verification: found %d/800 deep keys\n", found);
    ASSERT_EQ(found, 800);
...
static void *deep_sst_compactor_thread(void *arg)
{
    multi_cf_thread_data_t *d = (multi_cf_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        tidesdb_compact(d->data_cf);
        usleep(5000);
        tidesdb_compact(d->idx_cf);
        usleep(10000);
    }
    return NULL;
}

static void test_concurrent_multi_cf_deep_sst_mixed_ops(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ...
    ASSERT_EQ(tidesdb_create_column_family(db, "deep_data_cf", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "deep_idx_cf", &cf_config), 0);
    tidesdb_column_family_t *data_cf = tidesdb_get_column_family(db, "deep_data_cf");
    tidesdb_column_family_t *idx_cf = tidesdb_get_column_family(db, "deep_idx_cf");
    ASSERT_TRUE(data_cf != NULL);
    ASSERT_TRUE(idx_cf != NULL);

    /* we seed both CFs in batches with flush+compact between */
    for (int batch = 0; batch < 4; batch++)
    {
        for (int i = 0; i < 150; i++)
        {
            int key_idx = batch * 150 + i;
            tidesdb_txn_t *txn = NULL;
            ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);

            char pk[64], row_val[128], idx_key[128];
            snprintf(pk, sizeof(pk), "\x01dpk_%06d", key_idx);
            snprintf(row_val, sizeof(row_val), "deep_row_b%d_i%d", batch, i);
            snprintf(idx_key, sizeof(idx_key), "\x01didx_v%04d_pk_%06d", i % 50, key_idx);

            ASSERT_EQ(tidesdb_txn_put(txn, data_cf, (uint8_t *)pk, strlen(pk) + 1,
                                      (uint8_t *)row_val, strlen(row_val) + 1, 0),
                      0);
        ...
        tidesdb_flush_memtable(data_cf);
        tidesdb_flush_memtable(idx_cf);
        for (int w = 0; w < 200; w++)
        {
            int done = !atomic_load(&data_cf->is_flushing) && !atomic_load(&idx_cf->is_flushing) &&
                       queue_size(db->flush_queue) == 0;
            if (done) break;
            usleep(20000);
        }

        /* push data deeper down the trees */
        tidesdb_compact(data_cf);
        tidesdb_compact(idx_cf);
        for (int w = 0; w < 200; w++)
        {
            if (!atomic_load(&data_cf->is_compacting) && !atomic_load(&idx_cf->is_compacting))
                break;
            usleep(20000);
        }
    }

    /* we report level distribution */
    for (int c = 0; c < 2; c++)
    {
        tidesdb_column_family_t *cf = (c == 0) ? data_cf : idx_cf;
        const char *name = (c == 0) ? "data_cf" : "idx_cf";
        int nl = atomic_load(&cf->num_active_levels);
        int ts = 0, lws = 0;
        for (int l = 0; l < nl; l++)
        {
            if (cf->levels[l])
        ...
        printf("  %s: %d levels, %d SSTables, %d levels with data\n", name, nl, ts, lws);
    }
    ...
    pthread_t writers[3], readers[4], compactors[1];
    multi_cf_thread_data_t w_data[3], r_data[4], c_data[1];

    for (int i = 0; i < NUM_WRITERS; i++)
    {
        w_data[i] = (multi_cf_thread_data_t){db, data_cf, idx_cf, i, W_OPS, &errors, &go, &stop};
        pthread_create(&writers[i], NULL, multi_cf_writer_thread, &w_data[i]);
    }
    for (int i = 0; i < NUM_READERS; i++)
    {
        r_data[i] =
            (multi_cf_thread_data_t){db, data_cf, idx_cf, 10 + i, R_OPS, &errors, &go, &stop};
        pthread_create(&readers[i], NULL, multi_cf_reader_thread, &r_data[i]);
    }
    for (int i = 0; i < NUM_COMPACTORS; i++)
    {
        c_data[i] =
            (multi_cf_thread_data_t){db, data_cf, idx_cf, 30 + i, C_OPS, &errors, &go, &stop};
        pthread_create(&compactors[i], NULL, deep_sst_compactor_thread, &c_data[i]);
    }
    ...
    int final_errors = atomic_load(&errors);
    printf("  concurrent multi-CF deep SST mixed ops: errors=%d\n", final_errors);
    ASSERT_EQ(final_errors, 0);

    /* we verify deep seed keys (writers only touch keys 0-99, so 150+ should survive) */
    tidesdb_txn_t *vtxn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &vtxn), 0);
    int found = 0;
    ...
    tidesdb_txn_free(vtxn);
    printf("  post-stress data_cf verification: found %d/450 deep keys\n", found);
    ASSERT_EQ(found, 450);
...
typedef struct
{
    ...
    _Atomic(int) *go;
    _Atomic(int) *stop;
} mcf_sat_thread_data_t;

static void *mcf_sat_writer_thread(void *arg)
{
    mcf_sat_thread_data_t *d = (mcf_sat_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    }
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        char key[64], val[128];
        snprintf(key, sizeof(key), "sat_t%d_%06d", d->thread_id, i);
        snprintf(val, sizeof(val), "val_t%d_%06d_pad%0*d", d->thread_id, i, 60, 0);
...
static void test_multi_cf_flush_queue_saturation(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();

    const int NUM_CFS = 4;
    const int WRITERS_PER_CF = 2;
    const int OPS = 500;
    const char *cf_names[] = {"sat_cf0", "sat_cf1", "sat_cf2", "sat_cf3"};
    ...
    _Atomic(int) errors = 0, go = 0, stop = 0;
    const int TOTAL_THREADS = NUM_CFS * WRITERS_PER_CF;
    pthread_t threads[8];
    mcf_sat_thread_data_t tdata[8];

    for (int c = 0; c < NUM_CFS; c++)
    {
        for (int w = 0; w < WRITERS_PER_CF; w++)
        {
            int idx = c * WRITERS_PER_CF + w;
            tdata[idx] = (mcf_sat_thread_data_t){db, cfs[c], c * 10 + w, OPS, &errors, &go, &stop};
            pthread_create(&threads[idx], NULL, mcf_sat_writer_thread, &tdata[idx]);
        }
    }
    ...
    int final_errors = atomic_load(&errors);
    printf("  multi-CF flush queue saturation: errors=%d\n", final_errors);
    ASSERT_EQ(final_errors, 0);

    /* we verify data integrity across all CFs */
    for (int c = 0; c < NUM_CFS; c++)
    {
        tidesdb_txn_t *vtxn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &vtxn), 0);
        ...
        tidesdb_txn_free(vtxn);
        int expected = WRITERS_PER_CF * OPS;
        printf("  %s: found %d/%d keys\n", cf_names[c], found, expected);
...
{
    tidesdb_t *db;
    tidesdb_column_family_t *cf_a;
    tidesdb_column_family_t *cf_b;
    int thread_id;
    int num_ops;
    ...
    _Atomic(int) *go;
    _Atomic(int) *stop;
} mcf_atom_thread_data_t;

static void *mcf_atom_writer_thread(void *arg)
{
    mcf_atom_thread_data_t *d = (mcf_atom_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    }
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        char key_a[64], key_b[64], val[128];
        snprintf(key_a, sizeof(key_a), "atom_a_t%d_%06d", d->thread_id, i);
        snprintf(key_b, sizeof(key_b), "atom_b_t%d_%06d", d->thread_id, i);
        snprintf(val, sizeof(val), "xval_t%d_%06d_pad%0*d", d->thread_id, i, 50, 0);

        tidesdb_txn_put(txn, d->cf_a, (uint8_t *)key_a, strlen(key_a) + 1, (uint8_t *)val,
                        strlen(val) + 1, 0);
...
static void *mcf_atom_compactor_thread(void *arg)
{
    mcf_atom_thread_data_t *d = (mcf_atom_thread_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        tidesdb_compact(d->cf_a);
        tidesdb_compact(d->cf_b);
        usleep(10000);
    }
    return NULL;
}
...
static void test_multi_cf_cross_txn_compaction_atomicity(void)
{
    ...
    ASSERT_EQ(tidesdb_create_column_family(db, "atom_cf_a", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "atom_cf_b", &cf_config), 0);
    tidesdb_column_family_t *cf_a = tidesdb_get_column_family(db, "atom_cf_a");
    tidesdb_column_family_t *cf_b = tidesdb_get_column_family(db, "atom_cf_b");
    ASSERT_TRUE(cf_a != NULL);
    ASSERT_TRUE(cf_b != NULL);
    ...
    _Atomic(int) committed = 0, errors = 0, go = 0, stop = 0;

    pthread_t writers[4], compactors[2];
    mcf_atom_thread_data_t w_data[4], c_data[2];

    for (int i = 0; i < NUM_WRITERS; i++)
    {
        w_data[i] =
            (mcf_atom_thread_data_t){db, cf_a, cf_b, i, W_OPS, &committed, &errors, &go, &stop};
        pthread_create(&writers[i], NULL, mcf_atom_writer_thread, &w_data[i]);
    }
    for (int i = 0; i < NUM_COMPACTORS; i++)
    {
        c_data[i] = (mcf_atom_thread_data_t){db,         cf_a,    cf_b, 50 + i, C_OPS,
                                             &committed, &errors, &go,  &stop};
        pthread_create(&compactors[i], NULL, mcf_atom_compactor_thread, &c_data[i]);
    }
    ...
    int final_committed = atomic_load(&committed);
    int final_errors = atomic_load(&errors);
    ...
    for (int t = 0; t < NUM_WRITERS; t++)
    {
        for (int i = 0; i < W_OPS; i++)
        {
            ...
            snprintf(key_a, sizeof(key_a), "atom_a_t%d_%06d", t, i);
            snprintf(key_b, sizeof(key_b), "atom_b_t%d_%06d", t, i);
...
static void *mcf_iter_mut_iterator_thread(void *arg)
{
    mcf_iter_mut_data_t *d = (mcf_iter_mut_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    }
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        int cf_idx = (d->thread_id + i) % d->num_cfs;
        tidesdb_column_family_t *cf = d->cfs[cf_idx];
        ...
        {
            char sk[64];
            snprintf(sk, sizeof(sk), "lm_key_%06d", (i * 7) % 200);
            tidesdb_iter_seek(iter, (uint8_t *)sk, strlen(sk) + 1);
        }

        int walked = 0;
        while (tidesdb_iter_valid(iter) && walked < 30)
        {
            uint8_t *key = NULL;
            size_t ks = 0;
            tidesdb_iter_key(iter, &key, &ks);
...
static void *mcf_iter_mut_writer_thread(void *arg)
{
    mcf_iter_mut_data_t *d = (mcf_iter_mut_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        char key[64], val[128];
        snprintf(key, sizeof(key), "lm_key_%06d", (d->thread_id * 1000 + i) % 200);
        snprintf(val, sizeof(val), "lm_val_%06d_pad%0*d", i, 60, 0);
...
static void *mcf_iter_mut_flusher_thread(void *arg)
{
    mcf_iter_mut_data_t *d = (mcf_iter_mut_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        int cf_idx = i % d->num_cfs;
        tidesdb_flush_memtable(d->cfs[cf_idx]);
        usleep(5000);
    }
    return NULL;
}

static void *mcf_iter_mut_compactor_thread(void *arg)
{
    mcf_iter_mut_data_t *d = (mcf_iter_mut_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
...
static void test_multi_cf_iterator_during_level_mutation(void)
{
    ...
    const int NUM_CFS = 3;
    const char *cf_names[] = {"lm_cf0", "lm_cf1", "lm_cf2"};
    ...
    for (int c = 0; c < NUM_CFS; c++)
    {
        for (int i = 0; i < 200; i++)
        {
            ...
            ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
            char key[64], val[128];
            snprintf(key, sizeof(key), "lm_key_%06d", i);
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        for (int c = 0; c < NUM_CFS; c++)
        {
            if (atomic_load(&cfs[c]->is_flushing) || queue_size(db->flush_queue) > 0) busy = 1;
        }
        if (!busy) break;
        usleep(20000);
    }

    const int NUM_ITERATORS = 4;
    const int NUM_WRITERS = 3;
    const int NUM_FLUSHERS = 2;
    const int NUM_COMPACTORS = 2;
    ...
    pthread_t iter_threads[4], writer_threads[3], flusher_threads[2], compactor_threads[2];
    mcf_iter_mut_data_t i_data[4], w_data[3], f_data[2], c_data[2];

    for (int i = 0; i < NUM_ITERATORS; i++)
    {
        i_data[i] = (mcf_iter_mut_data_t){db, cfs, NUM_CFS, i, 600, &errors, &go, &stop};
        pthread_create(&iter_threads[i], NULL, mcf_iter_mut_iterator_thread, &i_data[i]);
    }
    for (int i = 0; i < NUM_WRITERS; i++)
    {
        w_data[i] = (mcf_iter_mut_data_t){db, cfs, NUM_CFS, 10 + i, 800, &errors, &go, &stop};
        pthread_create(&writer_threads[i], NULL, mcf_iter_mut_writer_thread, &w_data[i]);
    }
    for (int i = 0; i < NUM_FLUSHERS; i++)
    {
        f_data[i] = (mcf_iter_mut_data_t){db, cfs, NUM_CFS, 20 + i, 40, &errors, &go, &stop};
        pthread_create(&flusher_threads[i], NULL, mcf_iter_mut_flusher_thread, &f_data[i]);
    }
    for (int i = 0; i < NUM_COMPACTORS; i++)
    {
        c_data[i] = (mcf_iter_mut_data_t){db, cfs, NUM_CFS, 30 + i, 30, &errors, &go, &stop};
        pthread_create(&compactor_threads[i], NULL, mcf_iter_mut_compactor_thread, &c_data[i]);
    }
    ...
    for (int i = 0; i < NUM_WRITERS; i++) pthread_join(writer_threads[i], NULL);
    atomic_store(&stop, 1);
    for (int i = 0; i < NUM_ITERATORS; i++) pthread_join(iter_threads[i], NULL);
    for (int i = 0; i < NUM_FLUSHERS; i++) pthread_join(flusher_threads[i], NULL);
    for (int i = 0; i < NUM_COMPACTORS; i++) pthread_join(compactor_threads[i], NULL);

    int final_errors = atomic_load(&errors);
    printf("  multi-CF iterator during level mutation: errors=%d\n", final_errors);
    ASSERT_EQ(final_errors, 0);
...
static void test_multi_cf_cascading_memory_pressure(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    ...
    /* we use one writer per CF, they all start simultaneously */
    const int WRITERS = NUM_CFS;
    const int OPS = 400;
    pthread_t threads[6];
    mcf_sat_thread_data_t tdata[6];

    for (int i = 0; i < WRITERS; i++)
    {
        tdata[i] = (mcf_sat_thread_data_t){db, cfs[i], i, OPS, &errors, &go, &stop};
        pthread_create(&threads[i], NULL, mcf_sat_writer_thread, &tdata[i]);
    }
    ...
    int final_errors = atomic_load(&errors);
    printf("  multi-CF cascading memory pressure: errors=%d\n", final_errors);
    ASSERT_EQ(final_errors, 0);

    /* we verify each CF got all its data */
    for (int c = 0; c < NUM_CFS; c++)
    {
        tidesdb_txn_t *vtxn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &vtxn), 0);
...
{
    tidesdb_t *db;
    tidesdb_column_family_t *stable_a;
    tidesdb_column_family_t *stable_b;
    int thread_id;
    int num_ops;
    _Atomic(int) *errors;
    _Atomic(int) *go;
...
static void *mcf_churn_writer_thread(void *arg)
{
    mcf_churn_data_t *d = (mcf_churn_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    }
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        char key_a[64], key_b[64], val[128];
        snprintf(key_a, sizeof(key_a), "churn_a_t%d_%06d", d->thread_id, i);
        snprintf(key_b, sizeof(key_b), "churn_b_t%d_%06d", d->thread_id, i);
        snprintf(val, sizeof(val), "churn_val_%d_%d_pad%0*d", d->thread_id, i, 40, 0);

        tidesdb_txn_put(txn, d->stable_a, (uint8_t *)key_a, strlen(key_a) + 1, (uint8_t *)val,
                        strlen(val) + 1, 0);
...
static void *mcf_churn_lifecycle_thread(void *arg)
{
    mcf_churn_data_t *d = (mcf_churn_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    cfg.write_buffer_size = 1024;

    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        char name[64];
        snprintf(name, sizeof(name), "ephemeral_%d_%d", d->thread_id, i);

        if (tidesdb_create_column_family(d->db, name, &cfg) == 0)
        {
            tidesdb_column_family_t *ecf = tidesdb_get_column_family(d->db, name);
            if (ecf)
            {
                /* we write a few keys then drop */
                tidesdb_txn_t *txn = NULL;
                if (tidesdb_txn_begin(d->db, &txn) == 0)
                {
                    char key[64];
                    snprintf(key, sizeof(key), "eph_key_%d", i);
                    tidesdb_txn_put(txn, ecf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)"eph", 4,
                                    0);
                    tidesdb_txn_commit(txn);
                    tidesdb_txn_free(txn);
                }
                tidesdb_drop_column_family(d->db, name);
            }
        }
        usleep(5000);
    }
...
static void test_multi_cf_create_drop_churn_under_load(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    ...
    ASSERT_EQ(tidesdb_create_column_family(db, "stable_a", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "stable_b", &cf_config), 0);
    tidesdb_column_family_t *stable_a = tidesdb_get_column_family(db, "stable_a");
    tidesdb_column_family_t *stable_b = tidesdb_get_column_family(db, "stable_b");
    ASSERT_TRUE(stable_a != NULL);
    ASSERT_TRUE(stable_b != NULL);

    const int NUM_WRITERS = 3;
    const int NUM_CHURNERS = 2;
    const int W_OPS = 500;
    const int C_OPS = 30;

    _Atomic(int) errors = 0, go = 0, stop = 0;

    pthread_t writers[3], churners[2];
    mcf_churn_data_t w_data[3], c_data[2];

    for (int i = 0; i < NUM_WRITERS; i++)
    {
        w_data[i] = (mcf_churn_data_t){db, stable_a, stable_b, i, W_OPS, &errors, &go, &stop};
        pthread_create(&writers[i], NULL, mcf_churn_writer_thread, &w_data[i]);
    }
    for (int i = 0; i < NUM_CHURNERS; i++)
    {
        c_data[i] = (mcf_churn_data_t){db, stable_a, stable_b, 10 + i, C_OPS, &errors, &go, &stop};
        pthread_create(&churners[i], NULL, mcf_churn_lifecycle_thread, &c_data[i]);
    }

    atomic_store(&go, 1);
    for (int i = 0; i < NUM_WRITERS; i++) pthread_join(writers[i], NULL);
    atomic_store(&stop, 1);
    for (int i = 0; i < NUM_CHURNERS; i++) pthread_join(churners[i], NULL);

    int final_errors = atomic_load(&errors);
    printf("  CF create/drop churn under load: errors=%d\n", final_errors);
    ASSERT_EQ(final_errors, 0);

    /* we verify stable CFs retained all data */
    tidesdb_txn_t *vtxn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &vtxn), 0);
    ...
    for (int t = 0; t < NUM_WRITERS; t++)
    {
        ...
        {
            char ka[64], kb[64];
            snprintf(ka, sizeof(ka), "churn_a_t%d_%06d", t, i);
    ...
    tidesdb_txn_free(vtxn);
    int expected = NUM_WRITERS * W_OPS;
    printf("  stable_a: %d/%d  stable_b: %d/%d\n", found_a, expected, found_b, expected);
...
{
    tidesdb_t *db;
    tidesdb_column_family_t *cf_x;
    tidesdb_column_family_t *cf_y;
    int thread_id;
    int num_ops;
    tidesdb_isolation_level_t iso;
    _Atomic(int) *errors;
...
static void *mcf_iso_writer_thread(void *arg)
{
    mcf_iso_data_t *d = (mcf_iso_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    }
    ...
    {
        tidesdb_txn_t *txn = NULL;
        if (tidesdb_txn_begin_with_isolation(d->db, d->iso, &txn) != 0) continue;

        char key_x[64], key_y[64], val[128];
        snprintf(key_x, sizeof(key_x), "iso_x_t%d_%06d", d->thread_id, i);
        snprintf(key_y, sizeof(key_y), "iso_y_t%d_%06d", d->thread_id, i);
        snprintf(val, sizeof(val), "iso_val_%d_%d_pad%0*d", d->thread_id, i, 40, 0);

        tidesdb_txn_put(txn, d->cf_x, (uint8_t *)key_x, strlen(key_x) + 1, (uint8_t *)val,
                        strlen(val) + 1, 0);
...
static void *mcf_iso_reader_thread(void *arg)
{
    mcf_iso_data_t *d = (mcf_iso_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        /* we read from the assigned CF */
        tidesdb_column_family_t *cf = (d->thread_id % 2 == 0) ? d->cf_x : d->cf_y;

        /* we do a few point reads */
        for (int k = 0; k < 5; k++)
        {
            char key[64];
            int t = (i + k) % 4;
            snprintf(key, sizeof(key), "iso_%c_t%d_%06d", (cf == d->cf_x) ? 'x' : 'y', t,
                     (i * 3 + k) % 300);
            uint8_t *val = NULL;
            size_t vs = 0;
            if (tidesdb_txn_get(txn, cf, (uint8_t *)key, strlen(key) + 1, &val, &vs) == 0)
                free(val);
        }

        /* we do a short iterator scan */
        tidesdb_iter_t *iter = NULL;
        if (tidesdb_iter_new(txn, cf, &iter) == 0)
        {
            if (i % 2 == 0)
            ...
            int walked = 0;
            while (tidesdb_iter_valid(iter) && walked < 15)
            {
                uint8_t *key = NULL;
                size_t ks = 0;
                tidesdb_iter_key(iter, &key, &ks);
...
static void *mcf_iso_flusher_thread(void *arg)
{
    mcf_iso_data_t *d = (mcf_iso_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        tidesdb_flush_memtable((i % 2 == 0) ? d->cf_x : d->cf_y);
        usleep(8000);
    }
    return NULL;
}

static void test_multi_cf_mixed_isolation_cross_txn(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    ...
    ASSERT_EQ(tidesdb_create_column_family(db, "iso_cf_x", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "iso_cf_y", &cf_config), 0);
    tidesdb_column_family_t *cf_x = tidesdb_get_column_family(db, "iso_cf_x");
    tidesdb_column_family_t *cf_y = tidesdb_get_column_family(db, "iso_cf_y");
    ASSERT_TRUE(cf_x != NULL);
    ASSERT_TRUE(cf_y != NULL);

    _Atomic(int) errors = 0, go = 0, stop = 0;

    /* writer group -- SERIALIZABLE cross-CF txns */
    const int NUM_SER_WRITERS = 2;
    /* reader groups -- different isolation per CF */
    const int NUM_RC_READERS = 2; /* READ_COMMITTED on cf_x */
    const int NUM_RR_READERS = 2; /* REPEATABLE_READ on cf_y */
    const int NUM_FLUSHERS = 2;
    const int W_OPS = 500;
    const int R_OPS = 600;
    const int F_OPS = 30;

    pthread_t ser_writers[2], rc_readers[2], rr_readers[2], flushers[2];
    mcf_iso_data_t sw_data[2], rc_data[2], rr_data[2], f_data[2];

    for (int i = 0; i < NUM_SER_WRITERS; i++)
    {
        sw_data[i] = (mcf_iso_data_t){db,      cf_x, cf_y, i, W_OPS, TDB_ISOLATION_SERIALIZABLE,
                                      &errors, &go,  &stop};
        pthread_create(&ser_writers[i], NULL, mcf_iso_writer_thread, &sw_data[i]);
    }
    for (int i = 0; i < NUM_RC_READERS; i++)
    {
        rc_data[i] = (mcf_iso_data_t){
            db, cf_x, cf_y, 10 + i * 2, R_OPS, TDB_ISOLATION_READ_COMMITTED, &errors, &go, &stop};
        pthread_create(&rc_readers[i], NULL, mcf_iso_reader_thread, &rc_data[i]);
    }
    for (int i = 0; i < NUM_RR_READERS; i++)
    {
        rr_data[i] = (mcf_iso_data_t){
            db, cf_x, cf_y, 11 + i * 2, R_OPS, TDB_ISOLATION_REPEATABLE_READ, &errors, &go, &stop};
        pthread_create(&rr_readers[i], NULL, mcf_iso_reader_thread, &rr_data[i]);
    }
    for (int i = 0; i < NUM_FLUSHERS; i++)
    {
        f_data[i] = (mcf_iso_data_t){
            db, cf_x, cf_y, 30 + i, F_OPS, TDB_ISOLATION_READ_UNCOMMITTED, &errors, &go, &stop};
        pthread_create(&flushers[i], NULL, mcf_iso_flusher_thread, &f_data[i]);
    }

    atomic_store(&go, 1);

    for (int i = 0; i < NUM_SER_WRITERS; i++) pthread_join(ser_writers[i], NULL);
    atomic_store(&stop, 1);
    for (int i = 0; i < NUM_RC_READERS; i++) pthread_join(rc_readers[i], NULL);
    for (int i = 0; i < NUM_RR_READERS; i++) pthread_join(rr_readers[i], NULL);
    for (int i = 0; i < NUM_FLUSHERS; i++) pthread_join(flushers[i], NULL);
    ...
    for (int t = 0; t < NUM_SER_WRITERS; t++)
    {
        for (int i = 0; i < W_OPS; i++)
        {
            char kx[64], ky[64];
            snprintf(kx, sizeof(kx), "iso_x_t%d_%06d", t, i);
    ...
    tidesdb_txn_free(vtxn);
    printf("  iso verification: cf_x=%d cf_y=%d mismatches=%d\n", found_x, found_y, mismatch);
    ASSERT_EQ(mismatch, 0);
    ASSERT_EQ(found_x, found_y);

    tidesdb_close(db);
    cleanup_test_dir();
}

void test_db_stats_basic(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    ASSERT_NE(db, NULL);
    ...
    printf("  num_cfs=%d total_mem=%" PRIu64
           " resolved_limit=%zu pressure=%d pending=%d "
           "global_seq=%" PRIu64 "\n",
           stats.num_column_families, stats.total_memory, stats.resolved_memory_limit,
           stats.memory_pressure_level, stats.flush_pending_count, stats.global_seq);

    ASSERT_EQ(stats.num_column_families, 1);
    ASSERT_TRUE(stats.total_memory > 0);
    ASSERT_TRUE(stats.resolved_memory_limit > 0);
    ASSERT_TRUE(stats.global_seq > 0);
    ASSERT_EQ(stats.memory_pressure_level, 0);

    /* invalid args */
    ASSERT_NE(tidesdb_get_db_stats(NULL, &stats), 0);
    ASSERT_NE(tidesdb_get_db_stats(db, NULL), 0);
...
void test_purge_cf_basic(void)
{
    ...
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    cf_config.write_buffer_size = 1024;
    ASSERT_EQ(tidesdb_create_column_family(db, "purge_cf", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "purge_cf");
    ASSERT_NE(cf, NULL);
    ...
    {
        char key[32], val[128];
        snprintf(key, sizeof(key), "pkey_%04d", i);
        snprintf(val, sizeof(val), "pval_%04d_padding_to_make_it_bigger", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)val,
                                  strlen(val) + 1, -1),
                  0);
    }
    ...
    /* purge the CF */
    ASSERT_EQ(tidesdb_purge_cf(cf), 0);

    /* after purge, flushing and compacting should be idle */
    ...
    /* invalid args */
    ASSERT_NE(tidesdb_purge_cf(NULL), 0);
...
void test_purge_all(void)
{
    ...
    tidesdb_column_family_config_t cfg = tidesdb_default_column_family_config();
    cfg.write_buffer_size = 1024;
    ASSERT_EQ(tidesdb_create_column_family(db, "purge_a", &cfg), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "purge_b", &cfg), 0);
    tidesdb_column_family_t *cf_a = tidesdb_get_column_family(db, "purge_a");
    tidesdb_column_family_t *cf_b = tidesdb_get_column_family(db, "purge_b");
    ASSERT_NE(cf_a, NULL);
    ASSERT_NE(cf_b, NULL);
    ...
    {
        char key[32], val[128];
        snprintf(key, sizeof(key), "akey_%04d", i);
        snprintf(val, sizeof(val), "aval_%04d_padding_for_size", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf_a, (uint8_t *)key, strlen(key) + 1, (uint8_t *)val,
                                  strlen(val) + 1, -1),
                  0);
        snprintf(key, sizeof(key), "bkey_%04d", i);
        snprintf(val, sizeof(val), "bval_%04d_padding_for_size", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf_b, (uint8_t *)key, strlen(key) + 1, (uint8_t *)val,
                                  strlen(val) + 1, -1),
                  0);
    }
    ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    ...
    ASSERT_EQ(tidesdb_purge(db), 0);

    /* both CFs should be idle */
    ASSERT_FALSE(tidesdb_is_flushing(cf_a));
    ASSERT_FALSE(tidesdb_is_flushing(cf_b));
    ASSERT_FALSE(tidesdb_is_compacting(cf_a));
    ASSERT_FALSE(tidesdb_is_compacting(cf_b));

    /* db stats should show queues drained */
    tidesdb_db_stats_t stats;
    ASSERT_EQ(tidesdb_get_db_stats(db, &stats), 0);
    ASSERT_EQ(stats.flush_pending_count, 0);
    ASSERT_EQ((int)stats.flush_queue_size, 0);
    ASSERT_EQ((int)stats.compaction_queue_size, 0);
    ASSERT_EQ(stats.num_column_families, 2);
    printf("  after purge: sstables=%d data_size=%" PRIu64 "\n", stats.total_sstable_count,
           stats.total_data_size_bytes);

    /* data still readable from both CFs */
    txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    for (int i = 0; i < 30; i++)
    {
        ...
        size_t value_size = 0;
        snprintf(key, sizeof(key), "akey_%04d", i);
        ASSERT_EQ(tidesdb_txn_get(txn, cf_a, (uint8_t *)key, strlen(key) + 1, &value, &value_size),
                  0);
        free(value);
        snprintf(key, sizeof(key), "bkey_%04d", i);
        ASSERT_EQ(tidesdb_txn_get(txn, cf_b, (uint8_t *)key, strlen(key) + 1, &value, &value_size),
                  0);
        free(value);
    }
    tidesdb_txn_free(txn);
...
static void test_perf_multi_cf_commit_throughput(void)
{
    ...
    for (int c = 0; c < num_configs; c++)
    {
        ...
        {
            char name[32];
            snprintf(name, sizeof(name), "perf_cf_%d", i);
            ASSERT_EQ(tidesdb_create_column_family(db, name, &cf_config), 0);
            cfs[i] = tidesdb_get_column_family(db, name);
            ASSERT_TRUE(cfs[i] != NULL);
        }
        ...
        for (int t = 0; t < num_txns; t++)
        {
            ...
            {
                tidesdb_column_family_t *cf = cfs[op % num_cfs];
                char key[32], val[64];
                snprintf(key, sizeof(key), "k_%d_%d", t, op);
                snprintf(val, sizeof(val), "v_%d_%d", t, op);
                ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)val,
                                          strlen(val) + 1, -1),
                          0);
            }
        ...
        const double txns_per_sec = num_txns / (elapsed_ms / 1000.0);

        printf("\n  [PERF] %2d CFs: %d txns in %.1f ms (%.0f txn/s, %.1f us/txn)\n", num_cfs,
               num_txns, elapsed_ms, txns_per_sec, (elapsed_ms * 1000.0) / num_txns);
...
static void test_perf_savepoint_throughput(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    ASSERT_TRUE(db != NULL);
    ...
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    cf_config.write_buffer_size = 64 * 1024 * 1024;
    ASSERT_EQ(tidesdb_create_column_family(db, "sp_perf_cf", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "sp_perf_cf");
    ASSERT_TRUE(cf != NULL);

    const int num_ops_before_sp = 500;
    const int num_ops_after_sp = 500;
    const int num_iterations = 200;

    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);

    for (int i = 0; i < num_ops_before_sp; i++)
    {
        char key[32], val[64];
        snprintf(key, sizeof(key), "base_key_%04d", i);
        snprintf(val, sizeof(val), "base_val_%04d", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)val,
                                  strlen(val) + 1, -1),
                  0);
    }
    ...
    {
        ASSERT_EQ(tidesdb_txn_savepoint(txn, "bench_sp"), 0);

        for (int i = 0; i < num_ops_after_sp; i++)
        {
            char key[32], val[64];
            snprintf(key, sizeof(key), "sp_key_%04d_%04d", iter, i);
            snprintf(val, sizeof(val), "sp_val_%04d_%04d", iter, i);
            ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)val,
                                      strlen(val) + 1, -1),
                      0);
        }

        ASSERT_EQ(tidesdb_txn_rollback_to_savepoint(txn, "bench_sp"), 0);
    }

    clock_gettime(CLOCK_MONOTONIC, &t_end);
    const double elapsed_ms =
        (t_end.tv_sec - t_start.tv_sec) * 1000.0 + (t_end.tv_nsec - t_start.tv_nsec) / 1e6;
    const double iters_per_sec = num_iterations / (elapsed_ms / 1000.0);

    printf(
        "\n  [PERF] savepoint: %d create+%d puts+rollback x %d = %.1f ms "
        "(%.0f iter/s, %.1f us/iter)\n",
        num_ops_before_sp, num_ops_after_sp, num_iterations, elapsed_ms, iters_per_sec,
        (elapsed_ms * 1000.0) / num_iterations);
...
static void test_perf_txn_put_throughput(void)
{
    cleanup_test_dir();
    tidesdb_t *db = create_test_db();
    ASSERT_TRUE(db != NULL);
    ...
    {
        char key[32];
        snprintf(key, sizeof(key), "put_key_%08d", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, value, value_size, -1),
                  0);
    }
    ...
    const double elapsed_ms =
        (t_end.tv_sec - t_start.tv_sec) * 1000.0 + (t_end.tv_nsec - t_start.tv_nsec) / 1e6;
    const double ops_per_sec = num_ops / (elapsed_ms / 1000.0);

    printf(
        "\n  [PERF] txn_put: %d ops x %d-byte values in %.1f ms "
        "(%.0f ops/s, %.2f us/op)\n",
        num_ops, value_size, elapsed_ms, ops_per_sec, (elapsed_ms * 1000.0) / num_ops);

    ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    tidesdb_txn_free(txn);

    /* we drain flush queue before close */
    for (int w = 0; w < 200; w++)
    {
        usleep(50000);
        if (queue_size(db->flush_queue) == 0) break;
...
static tidesdb_t *create_unified_test_db(void)
{
    cleanup_test_dir();
...
static void test_unified_basic_put_get(void)
{
    tidesdb_t *db = create_unified_test_db();

    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf1", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "ucf1");
    ASSERT_TRUE(cf != NULL);

    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);

    uint8_t key[] = "unified_key";
    uint8_t value[] = "unified_value";
    ASSERT_EQ(tidesdb_txn_put(txn, cf, key, sizeof(key), value, sizeof(value), 0), 0);
    ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    tidesdb_txn_free(txn);
    ...
    uint8_t *rv = NULL;
    size_t rs = 0;
    ASSERT_EQ(tidesdb_txn_get(txn, cf, key, sizeof(key), &rv, &rs), 0);
    ASSERT_TRUE(rv != NULL);
    ASSERT_EQ(rs, sizeof(value));
    ASSERT_TRUE(memcmp(rv, value, sizeof(value)) == 0);
    free(rv);
    tidesdb_txn_free(txn);
...
static void test_unified_multi_cf_isolation(void)
{
    tidesdb_t *db = create_unified_test_db();

    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_a", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_b", &cf_config), 0);
    tidesdb_column_family_t *cf_a = tidesdb_get_column_family(db, "ucf_a");
    tidesdb_column_family_t *cf_b = tidesdb_get_column_family(db, "ucf_b");
    ASSERT_TRUE(cf_a != NULL);
    ASSERT_TRUE(cf_b != NULL);
    ...
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    ASSERT_EQ(tidesdb_txn_put(txn, cf_a, key, sizeof(key), val_a, sizeof(val_a), 0), 0);
    ASSERT_EQ(tidesdb_txn_put(txn, cf_b, key, sizeof(key), val_b, sizeof(val_b), 0), 0);
    ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    tidesdb_txn_free(txn);
    ...
    uint8_t *rv = NULL;
    size_t rs = 0;
    ASSERT_EQ(tidesdb_txn_get(txn, cf_a, key, sizeof(key), &rv, &rs), 0);
    ASSERT_EQ(rs, sizeof(val_a));
    ASSERT_TRUE(memcmp(rv, val_a, sizeof(val_a)) == 0);
    free(rv);

    rv = NULL;
    ASSERT_EQ(tidesdb_txn_get(txn, cf_b, key, sizeof(key), &rv, &rs), 0);
    ASSERT_EQ(rs, sizeof(val_b));
    ASSERT_TRUE(memcmp(rv, val_b, sizeof(val_b)) == 0);
    free(rv);
...
static void test_unified_delete(void)
{
    tidesdb_t *db = create_unified_test_db();

    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_del", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "ucf_del");

    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    uint8_t key[] = "del_key";
    uint8_t value[] = "del_value";
    ASSERT_EQ(tidesdb_txn_put(txn, cf, key, sizeof(key), value, sizeof(value), 0), 0);
    ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    tidesdb_txn_free(txn);
    ...
    uint8_t *rv = NULL;
    size_t rs = 0;
    ASSERT_EQ(tidesdb_txn_get(txn, cf, key, sizeof(key), &rv, &rs), TDB_ERR_NOT_FOUND);
    tidesdb_txn_free(txn);
...
static void test_unified_flush_to_sstable(void)
{
    cleanup_test_dir();
    ...
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    cf_config.write_buffer_size = 4096;
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_flush", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "ucf_flush");
    ASSERT_TRUE(cf != NULL);
    ...
    for (int i = 0; i < 200; i++)
    {
        ...
        char key[64];
        char value[128];
        snprintf(key, sizeof(key), "flush_key_%04d", i);
        snprintf(value, sizeof(value), "flush_value_%04d_padding_for_size", i);
    ...
    for (int i = 0; i < 200; i++)
    {
        ...
        char expected[128];
        snprintf(key, sizeof(key), "flush_key_%04d", i);
        snprintf(expected, sizeof(expected), "flush_value_%04d_padding_for_size", i);

        uint8_t *rv = NULL;
        size_t rs = 0;
        int rc = tidesdb_txn_get(txn, cf, (uint8_t *)key, strlen(key) + 1, &rv, &rs);
        ASSERT_EQ(rc, 0);
        ASSERT_TRUE(rv != NULL);
...
static void test_unified_recovery_after_reopen(void)
{
    ...
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_recov", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "ucf_recov");
    ...
    for (int i = 0; i < 50; i++)
    {
        ...
        char key[64];
        char value[64];
        snprintf(key, sizeof(key), "recov_key_%04d", i);
        snprintf(value, sizeof(value), "recov_value_%04d", i);
    ...
    cf = tidesdb_get_column_family(db, "ucf_recov");
    ASSERT_TRUE(cf != NULL);
    ...
    for (int i = 0; i < 50; i++)
    {
        ...
        char expected[64];
        snprintf(key, sizeof(key), "recov_key_%04d", i);
        snprintf(expected, sizeof(expected), "recov_value_%04d", i);
...
static void test_unified_multi_cf_many_keys(void)
{
    tidesdb_t *db = create_unified_test_db();

    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_mk1", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_mk2", &cf_config), 0);
    tidesdb_column_family_t *cf1 = tidesdb_get_column_family(db, "ucf_mk1");
    tidesdb_column_family_t *cf2 = tidesdb_get_column_family(db, "ucf_mk2");
    ASSERT_TRUE(cf1 != NULL && cf2 != NULL);
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        char key[64], val1[64], val2[64];
        snprintf(key, sizeof(key), "mk_key_%04d", i);
        snprintf(val1, sizeof(val1), "cf1_val_%04d", i);
        snprintf(val2, sizeof(val2), "cf2_val_%04d", i);

        ASSERT_EQ(tidesdb_txn_put(txn, cf1, (uint8_t *)key, strlen(key) + 1, (uint8_t *)val1,
                                  strlen(val1) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_put(txn, cf2, (uint8_t *)key, strlen(key) + 1, (uint8_t *)val2,
                                  strlen(val2) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_commit(txn), 0);
        tidesdb_txn_free(txn);
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        char key[64], exp1[64], exp2[64];
        snprintf(key, sizeof(key), "mk_key_%04d", i);
        snprintf(exp1, sizeof(exp1), "cf1_val_%04d", i);
        snprintf(exp2, sizeof(exp2), "cf2_val_%04d", i);

        uint8_t *rv = NULL;
        size_t rs = 0;

        ASSERT_EQ(tidesdb_txn_get(txn, cf1, (uint8_t *)key, strlen(key) + 1, &rv, &rs), 0);
        ASSERT_EQ(rs, strlen(exp1) + 1);
        ASSERT_TRUE(memcmp(rv, exp1, rs) == 0);
        free(rv);

        rv = NULL;
        ASSERT_EQ(tidesdb_txn_get(txn, cf2, (uint8_t *)key, strlen(key) + 1, &rv, &rs), 0);
        ASSERT_EQ(rs, strlen(exp2) + 1);
        ASSERT_TRUE(memcmp(rv, exp2, rs) == 0);
        free(rv);
...
static void test_unified_multi_cf_flush_and_recovery(void)
{
    cleanup_test_dir();
    ...
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    cf_config.write_buffer_size = 4096;
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_fr_a", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_fr_b", &cf_config), 0);
    ASSERT_EQ(tidesdb_create_column_family(db, "ucf_fr_c", &cf_config), 0);
    tidesdb_column_family_t *cfa = tidesdb_get_column_family(db, "ucf_fr_a");
    tidesdb_column_family_t *cfb = tidesdb_get_column_family(db, "ucf_fr_b");
    tidesdb_column_family_t *cfc = tidesdb_get_column_family(db, "ucf_fr_c");
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        char key[64], va[96], vb[96], vc[96];
        snprintf(key, sizeof(key), "fr_key_%04d", i);
        snprintf(va, sizeof(va), "val_a_%04d_padding_data", i);
        snprintf(vb, sizeof(vb), "val_b_%04d_padding_data", i);
        snprintf(vc, sizeof(vc), "val_c_%04d_padding_data", i);

        ASSERT_EQ(tidesdb_txn_put(txn, cfa, (uint8_t *)key, strlen(key) + 1, (uint8_t *)va,
                                  strlen(va) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_put(txn, cfb, (uint8_t *)key, strlen(key) + 1, (uint8_t *)vb,
                                  strlen(vb) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_put(txn, cfc, (uint8_t *)key, strlen(key) + 1, (uint8_t *)vc,
                                  strlen(vc) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_commit(txn), 0);
        tidesdb_txn_free(txn);
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        char key[64], ea[96], eb[96], ec[96];
        snprintf(key, sizeof(key), "fr_key_%04d", i);
        snprintf(ea, sizeof(ea), "val_a_%04d_padding_data", i);
        snprintf(eb, sizeof(eb), "val_b_%04d_padding_data", i);
        snprintf(ec, sizeof(ec), "val_c_%04d_padding_data", i);
    ...
    cfa = tidesdb_get_column_family(db, "ucf_fr_a");
    cfb = tidesdb_get_column_family(db, "ucf_fr_b");
...
typedef struct
{
    ...
    int num_ops;
    _Atomic(int) *commits_ok;
    _Atomic(int) *commits_fail;
    _Atomic(int) *go;
    _Atomic(int) *stop;
} unified_concurrent_data_t;

static void *unified_concurrent_writer(void *arg)
{
    unified_concurrent_data_t *d = (unified_concurrent_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    }
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        /* write to a random CF -- exercises unified memtable prefix isolation */
        int cf_idx = (d->thread_id + i) % d->num_cfs;
        tidesdb_column_family_t *cf = d->cfs[cf_idx];

        char key[64], value[128];
        /* shared keys across threads to stress concurrent overwrites */
        if (i % 4 == 0)
        {
            snprintf(key, sizeof(key), "ushared_%06d", i % 50);
        }
        else
        {
            snprintf(key, sizeof(key), "uuniq_t%d_%06d", d->thread_id, i);
        }

        size_t val_size = (i % 3 == 0) ? 96 : 48;
        snprintf(value, sizeof(value), "uval_t%d_i%d_cf%d", d->thread_id, i, cf_idx);

        tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value, val_size, 0);

        /* interleave cross-CF writes in same txn (every 5th op) */
        if (i % 5 == 0)
        {
            int cf2_idx = (cf_idx + 1) % d->num_cfs;
            char key2[64];
            snprintf(key2, sizeof(key2), "ucross_t%d_%06d", d->thread_id, i);
            tidesdb_txn_put(txn, d->cfs[cf2_idx], (uint8_t *)key2, strlen(key2) + 1,
                            (uint8_t *)value, val_size, 0);
        }
        ...
        {
            char del_key[64];
            snprintf(del_key, sizeof(del_key), "ushared_%06d", (i + 17) % 50);
...
static void *unified_concurrent_reader(void *arg)
{
    unified_concurrent_data_t *d = (unified_concurrent_data_t *)arg;
    while (!atomic_load(d->go))
    { /* spin */
    ...
    for (int i = 0; i < d->num_ops && !atomic_load(d->stop); i++)
    {
        ...
        for (int k = 0; k < 5; k++)
        {
            char key[64];
            if (k < 2)
                snprintf(key, sizeof(key), "ushared_%06d", (i * 3 + k) % 50);
            else
                snprintf(key, sizeof(key), "uuniq_t%d_%06d", (i + k) % 6, (i * 5 + k) % 300);
...
static void test_unified_concurrent_multi_cf_read_write(void)
{
    cleanup_test_dir();
    ...
    tidesdb_column_family_t *cfs[3];
    const char *cf_names[] = {"uconcur_cf0", "uconcur_cf1", "uconcur_cf2"};
    ...
    for (int c = 0; c < NUM_CFS; c++)
    {
        for (int i = 0; i < 50; i++)
        {
            ...
            snprintf(value, sizeof(value), "seed_cf%d_%06d", c, i);
            ASSERT_EQ(tidesdb_txn_put(txn, cfs[c], (uint8_t *)key, strlen(key) + 1,
                                      (uint8_t *)value, strlen(value) + 1, 0),
                      0);
            ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    ...
    printf("  unified concurrent: commits=%d errors=%d\n", final_ok, final_fail);
    ASSERT_EQ(final_fail, 0);
...
{
    cleanup_test_dir();
    const char *objstore_dir = "./test_objstore_evict";
    remove_directory(objstore_dir);
    ...
    tidesdb_objstore_config_t os_cfg = tidesdb_objstore_default_config();
    os_cfg.local_cache_max_bytes = 1; /* 1 byte limit forces immediate eviction */
    ...
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ASSERT_EQ(tidesdb_create_column_family(db, "evict_cf", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "evict_cf");
    ASSERT_TRUE(cf != NULL);
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
        char key[64], value[128];
        snprintf(key, sizeof(key), "ekey_%04d", i);
        snprintf(value, sizeof(value), "eval_%04d", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                                  strlen(value) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    ...
    /* with 1 byte cache limit, files should be evicted but data still readable
     * via range_get from the object store */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    int found = 0;
    for (int i = 0; i < 20; i++)
    ...
    tidesdb_txn_free(txn);
    printf("  paired eviction: read %d/20 keys after cache eviction\n", found);
    ASSERT_TRUE(found > 0);
...
static void test_objstore_stats(void)
{
    cleanup_test_dir();
    const char *objstore_dir = "./test_objstore_stats";
    remove_directory(objstore_dir);
    ...
    /* write enough data with large values to trigger multiple flushes */
    for (int i = 0; i < 50; i++)
    {
        tidesdb_txn_t *txn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    ...
    ASSERT_TRUE(stats.object_store_enabled == 1);
    ASSERT_TRUE(stats.total_uploads > 0);
    ASSERT_TRUE(stats.total_upload_failures == 0);
    ASSERT_TRUE(stats.replica_mode == 0);
    ASSERT_TRUE(stats.object_store_connector != NULL);
    printf("  objstore stats: enabled=%d uploads=%" PRIu64 " failures=%" PRIu64
           " connector=%s replica=%d\n",
           stats.object_store_enabled, stats.total_uploads, stats.total_upload_failures,
           stats.object_store_connector, stats.replica_mode);
...
static void test_objstore_frozen_point_lookup(void)
{
    cleanup_test_dir();
    const char *objstore_dir = "./test_objstore_frozen";
    remove_directory(objstore_dir);
    ...
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ASSERT_EQ(tidesdb_create_column_family(db, "frozen_cf", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "frozen_cf");
    ASSERT_TRUE(cf != NULL);
    ...
    for (int l = 0; l < num_levels; l++)
    {
        ...
        int num_ssts = atomic_load(&cf->levels[l]->num_sstables);
        tidesdb_sstable_t **ssts =
            atomic_load_explicit(&cf->levels[l]->sstables, memory_order_acquire);
        for (int s = 0; s < num_ssts; s++)
        {
            if (!ssts[s]) continue;
            /* close block managers if open */
            if (ssts[s]->klog_bm)
            {
                block_manager_close(ssts[s]->klog_bm);
                ssts[s]->klog_bm = NULL;
            }
            if (ssts[s]->vlog_bm)
            {
                block_manager_close(ssts[s]->vlog_bm);
                ssts[s]->vlog_bm = NULL;
            }
            /* delete local files */
            if (ssts[s]->klog_path) unlink(ssts[s]->klog_path);
            if (ssts[s]->vlog_path) unlink(ssts[s]->vlog_path);
        }
    }

    /* point lookups should still work via range_get from object store */
    tidesdb_txn_t *txn = NULL;
    ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
    int found = 0;
    for (int i = 0; i < 30; i++)
    ...
    tidesdb_txn_free(txn);
    printf("  frozen point lookup: read %d/30 keys via range_get\n", found);
    ASSERT_TRUE(found > 0);
...
{
    cleanup_test_dir();
    const char *objstore_dir = "./test_objstore_walsync";
    remove_directory(objstore_dir);
    ...
    tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
    ASSERT_EQ(tidesdb_create_column_family(db, "walsync_cf", &cf_config), 0);
    tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "walsync_cf");
    ASSERT_TRUE(cf != NULL);
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
        char key[64], value[128];
        snprintf(key, sizeof(key), "wskey_%04d", i);
        snprintf(value, sizeof(value), "wsval_%04d", i);
        ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                                  strlen(value) + 1, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_commit(txn), 0);
    ...
    tidesdb_objstore_t *store2 = tidesdb_objstore_fs_create(objstore_dir);
    ASSERT_TRUE(store2 != NULL);
    tidesdb_objstore_config_t os_cfg2 = tidesdb_objstore_default_config();
    tidesdb_config_t config2 = tidesdb_default_config();
    config2.db_path = TEST_DB_PATH;
    config2.object_store = store2;
    config2.object_store_config = &os_cfg2;
    config2.unified_memtable_write_buffer_size = 4096;

    db = NULL;
    ASSERT_EQ(tidesdb_open(&config2, &db), 0);

    cf = tidesdb_get_column_family(db, "walsync_cf");
    ASSERT_TRUE(cf != NULL);
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        uint8_t *val = NULL;
        size_t val_size = 0;
        if (tidesdb_txn_get(rtxn, cf, (uint8_t *)key, strlen(key) + 1, &val, &val_size) == 0 && val)
        {
            found++;
            free(val);
        }
    ...
    printf("  wal_sync_on_commit: cold start read %d/20 keys\n", found);
    ASSERT_TRUE(found > 0);
...
static void test_replica_mode(void)
{
    cleanup_test_dir();

    const char *objstore_dir = "./test_replica_store";
    remove_directory(objstore_dir);

    {
        ...
        tidesdb_objstore_config_t os_cfg = tidesdb_objstore_default_config();
        os_cfg.wal_sync_on_commit = 1; /* sync WAL on every commit for replica freshness */
        ...
        tidesdb_column_family_config_t cf_config = tidesdb_default_column_family_config();
        ASSERT_EQ(tidesdb_create_column_family(db, "replica_cf", &cf_config), 0);
        ...
        for (int i = 0; i < 30; i++)
        {
            ...
            ASSERT_EQ(tidesdb_txn_begin(db, &txn), 0);
            char key[64], value[128];
            snprintf(key, sizeof(key), "rkey_%04d", i);
            snprintf(value, sizeof(value), "rval_%04d", i);
            ASSERT_EQ(tidesdb_txn_put(txn, cf, (uint8_t *)key, strlen(key) + 1, (uint8_t *)value,
                                      strlen(value) + 1, 0),
                      0);
            ASSERT_EQ(tidesdb_txn_commit(txn), 0);
...

    {
        ...
        os_cfg.replica_sync_interval_us = 100000; /* 100ms for fast test */
        ...
        /* cold start should have discovered the CF */
        tidesdb_column_family_t *cf = tidesdb_get_column_family(db, "replica_cf");
        ASSERT_TRUE(cf != NULL);
        ...
        tidesdb_txn_free(txn);
        printf("  replica: read back %d/30 keys\n", found);
        ASSERT_TRUE(found > 0);

        /* verify writes are rejected */
        tidesdb_txn_t *wtxn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &wtxn), 0);
        int put_rc = tidesdb_txn_put(wtxn, cf, (uint8_t *)"blocked", 8, (uint8_t *)"nope", 5, 0);
        ASSERT_EQ(put_rc, TDB_ERR_READONLY);
        printf("  replica: write correctly rejected with TDB_ERR_READONLY\n");
        tidesdb_txn_free(wtxn);

        /* we promote to primary and verify writes work */
        ASSERT_EQ(tidesdb_promote_to_primary(db), 0);

        tidesdb_txn_t *ptxn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &ptxn), 0);
        ASSERT_EQ(tidesdb_txn_put(ptxn, cf, (uint8_t *)"promoted_key", 13,
                                  (uint8_t *)"promoted_val", 13, 0),
                  0);
        ASSERT_EQ(tidesdb_txn_commit(ptxn), 0);
        tidesdb_txn_free(ptxn);
        printf("  replica: promoted to primary, write succeeded\n");

        /* verify the promoted write is readable */
        tidesdb_txn_t *rtxn = NULL;
        ASSERT_EQ(tidesdb_txn_begin(db, &rtxn), 0);
        uint8_t *pval = NULL;
        size_t pval_size = 0;
        ASSERT_EQ(tidesdb_txn_get(rtxn, cf, (uint8_t *)"promoted_key", 13, &pval, &pval_size), 0);
...
int main(int argc, char **argv)
{
    ...
    RUN_TEST(test_concurrent_iter_seek_directions, tests_passed);
    RUN_TEST(test_concurrent_multi_cf_put_delete_iterate, tests_passed);
    RUN_TEST(test_iterator_reuse_seek_patterns, tests_passed);
    RUN_TEST(test_concurrent_savepoint_rollback_under_load, tests_passed);
    RUN_TEST(test_concurrent_iter_seek_directions_deep_sst, tests_passed);
    ...
    RUN_TEST(test_perf_txn_put_throughput, tests_passed);
    RUN_TEST(test_unified_basic_put_get, tests_passed);
