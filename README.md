...
- Efficient seek operations using O(log n) skip list positioning and optional block indexes for prefix boundary to block mapping in SSTables. Block indexes sample every Nth block min-max prefix boundary (default N=1) during SSTable creation to build a sparse index that maps sampled ranges to block numbers to optimize reads.
