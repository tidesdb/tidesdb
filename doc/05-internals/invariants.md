...
| The log a rotation installs is opened before the rotation, off the lock | [Memtable and WAL](/internals/memtable-and-wal) | **Unguarded** — nothing asserts a rotation does no file creation, and opening one inline costs a file creation and a flush thread with every committer stopped -- a cost set by the filesystem, not by the engine, and on a slow one it dwarfs the rest of the rotation. The engine logs a slow prepare separately from a slow rotation, and times the log open on its own inside both, so a rotation that overran can be read as the open or as the rest rather than as one number covering both |
