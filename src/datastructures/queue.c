 ...
 * deviation from the no-steady-state-allocation rule for an unbounded queue whose depth the caller
 * does not cap */

/**
 * queue_alloc_node
 * take a node from the pool, or allocate one from the heap when the pool is empty
 * @param queue the queue whose pool to draw from, pool access guarded by pool_lock
...
static inline queue_node_t *queue_alloc_node(queue_t *queue)
{
    ...
    /* the pool is the common source, so draw from it before falling back to the heap */
...
/**
 * queue_free_node
 * return a node to the pool, or free it to the heap once the pool is full
 * @param queue the queue whose pool to return to, pool access guarded by pool_lock
...
static inline void queue_free_node(queue_t *queue, queue_node_t *node)
{
     ...
     * racy read is safe           -- worst case a node is freed when pool had room */
...
queue_t *queue_new(void)
{
    ...
    /* the dummy node separates head and tail so enqueue and dequeue take independent locks */
...
int queue_enqueue(queue_t *queue, void *data)
{
    ...
    /* enqueue touches only the tail, so it takes tail_lock alone */
    pthread_mutex_lock(&queue->tail_lock);

    /* bump size before publishing the node. if the node were published first, a concurrent
...
static inline void *queue_dequeue_internal(queue_t *queue)
{
    ...
    /* advance past the old dummy; the consumed node becomes the new dummy */
...
/* take the item under the reader-exclusive lock, returning it, or NULL when the queue is empty */
static void *queue_take_locked(queue_t *queue)
{
...
void *queue_dequeue(queue_t *queue)
{
    ...
    return queue_take_locked(queue);
}

/**
 * queue_block_until_ready
 * block on head_lock, held by the caller, until an item arrives or the queue shuts down
 * registers as a waiter for the duration so queue_free_with_data can join, and wakes a draining
 * free when the last waiter leaves; each sleep is time-bounded so a missed signal cannot hang the
 * thread
 * @param queue the queue, with head_lock held
 */
static void queue_block_until_ready(queue_t *queue)
{
    atomic_fetch_add_explicit(&queue->waiter_count, 1, memory_order_relaxed);

    /* condition-bounded rather than statically bounded -- the timed wait is the liveness backstop
     * and shutdown breaks the predicate */
    ...
    /* the last waiter to leave wakes a queue_free_with_data that is draining waiters */
    if (remaining == 0) pthread_cond_broadcast(&queue->not_empty);
}
...
void *queue_dequeue_wait(queue_t *queue)
{
    ...
    /* spin briefly on the common non-empty case before paying for a blocking wait */
    for (int i = 0; i < QUEUE_SPIN_COUNT; i++)
    {
        if (atomic_load_explicit(&queue->size, memory_order_acquire) > 0)
        {
            void *data = queue_take_locked(queue);
    ...
    pthread_mutex_lock(&queue->head_lock);
    queue_block_until_ready(queue);

    /* another consumer can steal the item between the wait and reacquiring the reader-exclusive
     * lock, so recheck under the lock each pass and re-block when the item is gone */
    while (1)
    {
...
int queue_clear(queue_t *queue)
{
    ...
    /* the reader-exclusive lock plus both end locks give full exclusivity over the node chain */
    pthread_rwlock_wrlock(&queue->read_lock);
    pthread_mutex_lock(&queue->head_lock);
    pthread_mutex_lock(&queue->tail_lock);

    /* free every node after the dummy; the exclusive locks make relaxed ordering sufficient */
...
void *queue_peek_at(queue_t *queue, const size_t index)
{
    ...
    {
        queue_node_t *next = atomic_load_explicit(&current->next, memory_order_acquire);
        /* prefetch the next node to overlap its memory latency with this iteration */
        if (QUEUE_LIKELY(next != NULL))
        {
            PREFETCH_READ(next);
        }
...
void queue_shutdown(queue_t *queue)
{
    ...
    /* set the shutdown flag and wake every blocked waiter so each returns NULL */
...
void queue_free_with_data(queue_t *queue, void (*free_fn)(void *))
{
    ...
    /* shut down and wake every waiter, then join them before tearing down the primitives they touch
     */
    atomic_store_explicit(&queue->shutdown, 1, memory_order_release);
    ...
    /* the timed wait tolerates a signal lost on platforms where broadcast can race a late waiter */
    ...
    /* free every node including the dummy, applying free_fn to the payloads when given */
