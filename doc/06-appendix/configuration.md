...
| `memtable_l0_queue_stall_threshold` | 16 | Sealed-memtable queue depth at which ingest is paced; writers throttle in a band below it and are held at it, always with a ceiling. It also sets the L1 tier's marks — dwell from half of it, wait at it — so it governs **two of three** admission signals. The staging ring is the third and paces independently on `memtable_write_buffer_size`, so raising this may still not reduce stalling |
