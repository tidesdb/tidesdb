...
- Hybrid LRU/LFU block cache (80% LRU, 20% LFU) shared across all column families and SSTables with atomic reference counting for safe concurrent access. Configurable memory budget (default 64MB) automatically converted to block capacity based on weighted average block size (70% klog at 64KB, 30% vlog at 4KB = ~46KB average). Blocks promoted to LFU tier after 2 accesses with 60-second TTL. Cache keys use format `cf_name:sstable_id:block_type:offset` for precise block identification. Atomic reference acquisition via `lru_cache_get_copy_n` prevents race conditions where blocks could be evicted while in use, eliminating heap-use-after-free bugs in concurrent read paths. Blocks are cached on first read and automatically evicted when memory pressure occurs, significantly reducing disk I/O for hot data while maintaining thread-safety across iterators, GET operations, and compaction.
