...
| A family is freed only once every view that named it has no borrow left | [Architecture](/internals/architecture) | **ThreadSanitizer** -- `cf_free` against a flush still reading the handle. Waiting only on the view the removal displaced leaves a borrow of an older view holding it, which no functional test detects |
